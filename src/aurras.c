...
int main(int argc, char* argv[]){
    ...
    int nb_read;
    char* ignore = malloc(sizeof(char) * STD_SIZE);
    ...
    if (argc > 1){
        if (strcmp(argv[1],"status") == 0){
            ...
            write(client_server, ignore, STD_SIZE);
            while(!read_flag && (nb_read = read(server_client, status, STD_SIZE)) > 0){
